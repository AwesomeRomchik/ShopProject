...
<?php # DISPLAY COMPLETE PRODUCTS PAGE.
 ...
 $q = "SELECT * FROM shop WHERE gender='men'" ;
 if(isset($_POST['colour'])){
     if($_POST['colour'] == 'black'){
         //Run query
         $q = "SELECT * FROM shop WHERE colour = 'black' AND gender='men'";
     }elseif($_POST['colour'] == 'beige'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'beige' AND gender='men'";
     }elseif($_POST['colour'] == 'blue'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'blue' AND gender='men'";
     }elseif($_POST['colour'] == 'green'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'green' AND gender='men'";
     }elseif($_POST['colour'] == 'mix'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'mix' AND gender='men'";
     }elseif($_POST['colour'] == 'grey'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'grey' AND gender='men'";
     }elseif($_POST['colour'] == 'purple'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'purple' AND gender='men'";
     }elseif($_POST['colour'] == 'yellow'){
         //Run query
         $q = "SELECT * FROM shop WHERE COLOUR = 'yellow' AND gender='men'";
  ...
  elseif(isset($_POST['item_type'])){
      if($_POST['item_type'] == 'shoe'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='shoe' AND gender='men'";
      }elseif($_POST['item_type'] == 'shorts'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='shorts' AND gender='men'";    
      }elseif($_POST['item_type'] == 'tee'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='tee' AND gender='men'";
      }elseif($_POST['item_type'] == 'jacket'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='jacket' AND gender='men'";
      }elseif($_POST['item_type'] == 'windbreaker'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='windbreaker' AND gender='men'";
      }elseif($_POST['item_type'] == 'tanktop'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='tanktop' AND gender='men'";
      }elseif($_POST['item_type'] == 'skirt'){
          //Run query
          $q = "SELECT * FROM shop WHERE item_type='skirt' AND gender='men'";
      }else{
     $query = "SELECT * FROM shop WHERE gender='men' AND gender='men'";
 }
 }
 elseif(isset($_POST['price'])){
     if($_POST['price'] == 'mintomax'){
         //Run query
         $q = "SELECT * FROM shop WHERE gender='men' ORDER BY item_price ASC";
     }elseif($_POST['price'] == 'maxtomin'){
         //Run query
         $q = "SELECT * FROM shop WHERE gender='men' ORDER BY item_price DESC";
     }
 }
 $r = mysqli_query( $dbc, $q ) ;
 if ( mysqli_num_rows( $r ) > 0 )
 {
  # Display body section.
     ...
     while ( $row = mysqli_fetch_array( $r, MYSQLI_ASSOC ))
     {
         ...
         //echo '<form><button type="button" class="btn btn-Default"><a style=\"color:black;\" href="added.php?id='.$row['item_id'].'">Add To Cart</a></button></form></div></div>';
     }
     echo '';
